...
function LocationInput({
  label, icon, value, onChange, onSelect, onGPS, gpsLoading, nearest,
}: {
  ...
  const handleSelect = (s: SuggestResult) => {
    onSelect(s);
    setSuggestions([]);
    setOpen(false);
  };
    ...
    <div className="relative" ref={wrapperRef}>
      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          />
          {searching && (
            <div className="absolute right-3 top-3.5 text-xs animate-pulse" style={{ color: "#1B2B6B" }}>🔍</div>
          )}
          ...
          {open && suggestions.length > 0 && (
            <div className="absolute z-50 w-full mt-1 rounded-xl shadow-2xl border overflow-hidden" style={{ background: "#fff", borderColor: "#E2E8F0" }}>
              {suggestions.map((s, i) => (
                <button
                  key={i}
                  className="w-full text-left px-4 py-3 hover:bg-blue-50 transition border-b last:border-0"
                      ...
                      {s.type === "place" && (
                        <div className="flex items-center gap-1 mt-1">
                          ...
                          <span className="text-xs" style={{ color: "#94A3B8" }}>
                            {s.nearest_stop.stop_name} · {s.nearest_stop.distance_m}m
                          </span>
                        </div>
                      )}
                    </div>
      ...
      {nearest && (
        <div className="mt-1.5 text-xs flex items-center gap-1.5" style={{ color: "#64748B" }}>
          ...
          {nearest.stop_name}
          {nearest.distance_m > 0 && ` · ${nearest.distance_m}m dari lokasi`}
        </div>
      )}
    </div>
  );
...
export default function Home() {
  ...
  const searchRoute = async () => {
    ...
    if (!fromQuery || !toQuery) {
      setError("Pilih lokasi asal dan tujuan dari dropdown dulu ya!");
      return;
    }
    setLoading(true);
    setError("");
      ...
      <div className="max-w-2xl mx-auto px-4 -mt-2">
        <div className="rounded-2xl p-5 mb-4 shadow-lg" style={{ background: "#fff" }}>
          <div className="space-y-4">
            <LocationInput
              label="Lokasi Asal" icon="🔵" value={from}
            ...
            <LocationInput
              label="Lokasi Tujuan" icon="🔴" value={to}
                ...
                <div key={i}>
                  {step.change_to && (
                    <div className="flex items-center gap-2 py-2 pl-6">
                      <span className="text-xs font-semibold px-2.5 py-1 rounded-full text-white" style={{ background: AGENCY_BADGE[step.change_agency || "TJ"]?.bg }}>
                  ...
                  <div className="flex items-center gap-3 py-1.5">
                    <div style={{ width: 20 }} className="flex justify-center">
                      <div className="w-3 h-3 rounded-full border-2 border-white shadow" style={{
